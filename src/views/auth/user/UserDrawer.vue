...
<script lang="ts">
  ...
  import { addUser, editUser, getDepartments, getUserType } from '/@/api/auth/user';
    ...
    setup(_, { emit }) {
      ...
      const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
        ...
        updateSchema({
          field: 'deptNo',
          componentProps: {
            options: departs.map(({ deptName, deptNo }) => {
              return {
                lable: deptName,
                value: deptNo,
              };
        ...
        console.log('userTypes:',userTypes.value);
        updateSchema({
          field: 'userTypeNo',
          componentProps:{
        ...
        updateSchema({
          field: 'userNo',
          dynamicDisabled: unref(isUpdate) ? true : false,
        });
